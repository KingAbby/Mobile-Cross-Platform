        ...
        if (cameraRef) {
            try {
                ...
                try {
                    ...
                    } console.log('Photo metadata saved to Supabase database:', dbData);                    // Show success notification
                    await scheduleLocalNotification(
                        'Photo Uploaded Successfully',
                        `Location: ${location ? `Lat: ${location.coords.latitude}, Long: ${location.coords.longitude}` : 'No location data'}`
                    );
                } catch (dbException) {
                    console.error('Exception during database operation:', dbException);                    // Show failure notification
                    await scheduleLocalNotification(
                        'Upload Failed',
